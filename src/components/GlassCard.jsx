...
const GlassCard = () => {
  ...
  return (
    <div className="bg-opacity-25 backdrop-blur bg-gray-700 bg-blur-lg border border-gray-200 rounded-lg overflow-hidden max-w-full md:max-w-md mx-4 my-0 p-6 flex flex-col items-center text-center">
      <img src={imageUrl} alt={title} className="w-32 h-32 rounded-full mb-4" />
      <div>
        <h2 className="text-xl font-bold text-red-200">{title}</h2>
        <p className="text-white font-mono max-h-40 overflow-hidden overflow-y-auto">
          {description}
        </p>
      </div>
    </div>
