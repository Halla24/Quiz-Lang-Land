...
const GlassCard = () => {
  ...
  return (
    <div className="bg-opacity-25 bg-gray-900 bg-blur-lg border border-gray-200 rounded-lg overflow-hidden w-100 h-80 mx-4 my-0 p-100 flex flex-col items-center text-center">
      <img src={imageUrl} alt={title} className="w-32 h-32 rounded-full mb-4" />
      <div>
        <h2 className="text-xl font-bold text-red-200">{title}</h2>
        <p className="text-black font-mono h-50 ">{description}</p>
      </div>
    </div>
  );
};
